...
WORKDIR /src
COPY ["PortfolioTracker.csproj", "."]
RUN dotnet restore "./PortfolioTracker.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./PortfolioTracker.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./PortfolioTracker.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "PortfolioTracker.dll"]